...
export default function BlogPage({ onBackToHome, onOpenGuide, onOpenLegal, initialArticleId }) {
  ...
  const otherArticle = BLOG_POSTS.find(p => p.id !== selectedArticleId) || BLOG_POSTS[1];
    ...
    <div className="min-h-screen bg-[#0b0c10] text-[#eae8e3] font-sans antialiased selection:bg-[#c5a059]/30 selection:text-[#fbf9f4]">
      ...
      <div className="bg-[#12141d] border-b border-white/10 px-4 py-3">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-[#dfc58b] font-semibold uppercase tracking-wider">
            <Layers className="w-4 h-4 text-[#c5a059]" />
            <span>Select Authority Post:</span>
          </div>
            ...
            {BLOG_POSTS.map((post, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-left text-xs transition-all cursor-pointer border flex items-center gap-2.5 ${
                    isSelected
                      ? 'bg-[#202436] border-[#c5a059] text-[#fbf9f4] shadow-md'
                ...
                >
                  ...
                  <span className="font-serif font-semibold truncate max-w-[280px] sm:max-w-[240px]">
                    {post.id === 'nitric-oxide-blueprint-nutrition' ? 'Nitric Oxide Blueprint & Diet' : 'Pelvic Floor & Reverse Kegels'}
                  </span>
                </button>
              );
            })}
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <article className="lg:col-span-8">
            ...
            ) : selectedArticleId === 'psychology-of-stamina-mental-control' ? (
            ...
            <div className="mt-12 p-6 rounded-2xl bg-[#141624] border border-[#c5a059]/40 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="space-y-1 text-center sm:text-left">
                <div className="text-xs text-[#dfc58b] font-semibold uppercase tracking-wider">
                  Continue Reading
                </div>
              ...
              {BLOG_POSTS.map(p => (
                <li key={p.id}>
                  <button 
                    ...
                    className="hover:text-[#dfc58b] transition-colors cursor-pointer text-left"
                  >
                    {p.id === 'nitric-oxide-blueprint-nutrition' ? 'Nitric Oxide Blueprint' : 'Pelvic Floor & Stamina'}
                  </button>
                </li>
              ))}
              <li>
